...
cls_labels_dict = np.load('./datasets/CoCA/CoCA.npy', allow_pickle=True).item()
...
def load_image_label_from_xml(img_name, cosal_root):
    from xml.dom import minidom

    elem_list = minidom.parse(os.path.join(cosal_root, ANNOT_FOLDER_NAME, decode_int_filename(img_name) + '.xml')).getElementsByTagName('name')
...
def load_image_label_list_from_xml(img_name_list, cosal_root):

    return [load_image_label_from_xml(img_name, cosal_root) for img_name in img_name_list]
...
def load_image_label_list_from_npy(img_name_list):
    ...
    #return np.array(cls_labels_dict)

# def get_img_path(img_name, cosal_root):
#     if not isinstance(img_name, str):
#         img_name = decode_int_filename(img_name)
#     return os.path.join(cosal_root, IMG_FOLDER_NAME, img_name + '.jpg')
#######
def get_img_path(img_name, cosal_root):
    if not isinstance(img_name, str):
        img_name = decode_int_filename(img_name)
    return os.path.join(cosal_root, img_name + '.jpg')

def load_img_name_list(dataset_path):

    #img_name_list = np.loadtxt(dataset_path, dtype=np.float32)
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = np.asarray(imageio.imread(get_img_path(name_str, self.cosal_root)))
...
class VOC12ClassificationDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, cosal_root,
        ...
        super().__init__(img_name_list_path, cosal_root,
...
#class VOC12ClassificationDatasetMSF(Dataset):
#
#    def __init__(self, img_name_list_path, cosal_root,
#                 img_normal=TorchvisionNormalize(),
#                 scales=(1.0,)):
#        self.scales = scales
#        super().__init__(img_name_list_path, cosal_root, img_normal=img_normal)
#        self.scales = scales
#
#    def __getitem__(self, idx):
#        name = self.img_name_list[idx]
#        name_str = decode_int_filename(name)
#
#        img = imageio.imread(get_img_path(name_str, self.cosal_root))
#
#        ms_img_list = []
...
class VOC12SegmentationDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, label_dir, crop_size, cosal_root,
...
class VOC12AffinityDataset(VOC12SegmentationDataset):
    def __init__(self, img_name_list_path, label_dir, crop_size, cosal_root,
        ...
        super().__init__(img_name_list_path, label_dir, crop_size, cosal_root, rescale, img_normal, hor_flip, crop_method=crop_method)
